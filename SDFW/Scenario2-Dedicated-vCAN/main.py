...
def log_to_vcan7(message):
    """Send log messages to vcan7"""
    if 'vcan7' in INTERFACES:
        try:
            INTERFACES['vcan7'].send(message)
        except can.CanError:
            log_msg = f"Failed to send log message on vcan7"
            print(log_msg)
            logging.error(log_msg)
        ...
        while True:
            for interface, bus in INTERFACES.items():
                ...
                if message:
                    ...
                    # Log every message to vcan7
                    log_to_vcan7(message)
